package com.eagletsoft.circlemenu;
...
public class CircleMenu extends View {
    ...
    //markhe: skip animation when in eink
    ...
    private int getClickMenuColor() {
        return clickIndex == 0 ? mainMenuColor : subMenuList.get(clickIndex - 1).color;
    }
    ...
    private void drawCircleIcon(Canvas canvas) {
        canvas.save();
        Drawable selDrawable = subMenuList.get(clickIndex - 1).drawable;
    ...
    private void drawSubMenu(Canvas canvas) {
        ...
        for (int i = 0; i < itemNum; i++) {
            angle = i * (360 / itemNum);
            int color = subMenuList.get(i).color;
            if (status == STATUS_MENU_OPEN) {
                ...
                oPaint.setColor(calcAlphaColor(color, false));
                sPaint.setColor(calcAlphaColor(color, false));
                ...
                oPaint.setColor(calcAlphaColor(color, true));
                sPaint.setColor(calcAlphaColor(color, true));
            } else {
    ...
    private void drawSubMenuIcon(Canvas canvas, int centerX, int centerY, int index) {
        ...
        resetBoundsAndDrawIcon(canvas, subMenuList.get(index).drawable, centerX, centerY, diff);
    }
    ...
    private int calcPressedEffectColor(int menuIndex, float depth) {
        int color = menuIndex == 0 ? mainMenuColor : subMenuList.get(menuIndex - 1).color;
     ...
     * @return
     */
    public CircleMenu addSubMenu(String key, int menuColor, int menuRes) {
        if (subMenuList.size() < MAX_SUBMENU_NUM) {
            subMenuList.add(new SubMenu(key, convertDrawable(menuRes), menuColor));
            itemNum = subMenuList.size();
        }
        return this;
    }
     ...
     * @return
     */
    public CircleMenu addSubMenu(String key, int menuColor, Bitmap menuBitmap) {
        if (subMenuList.size() < MAX_SUBMENU_NUM) {
            subMenuList.add(new SubMenu(key, convertBitmap(menuBitmap), menuColor));
            itemNum = subMenuList.size();
        }
        return this;
    }
     ...
     * @return
     */
    public CircleMenu addSubMenu(String key, int menuColor, Drawable menuDrawable) {
        if (subMenuList.size() < MAX_SUBMENU_NUM) {
            subMenuList.add(new SubMenu(key, menuDrawable, menuColor));
            itemNum = subMenuList.size();
        }
        return this;
    }
    ...
    public class SubMenu {
        ...
        public SubMenu(String key, Drawable drawable, int color) {
            this.key = key;
